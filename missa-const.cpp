...
int main( int argc, char **argv )
{
   ...
   init_distributions[1][4] = 1.0;
   
   // Markov chains
   markov_chain MC1(P, init_distributions[0]);
   markov_chain MC2(P, init_distributions[1]);
   ...
   // MISSA object
   MISSA missa_op(x.size(), nchains, nthreads, mode);
   ...
   for ( int test = 1; test < 2; test++ ){
	   ...
	   for ( int k = 0; k < niters; k++ ){
		  ...
		  next_states[0] = MC1.eval_MC_time(k, cur_states[0]);
	      ...
	      // Optimization step
	      missa_op.apply( lambda, next_states, eps, x );
	  ...
	  */
	  ...
	  char buffer [500];
	  int str = std::sprintf(buffer, "/home/rmassambone/Documentos/Elias-Eduardo/MISSA-testes/results/constant-stepsize/best-for-each/objective_MISSA_test-%d.txt", test );
	  FILE * arq;
	  arq = std::fopen (buffer, "w" );
