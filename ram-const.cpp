...
int main( int argc, char **argv )
{
   ...
   // Neighbors
   N[0].push_back(1); N[0].push_back(2);
   N[1].push_back(0); N[1].push_back(2); N[1].push_back(6);
   N[2].push_back(0); N[2].push_back(1); N[2].push_back(5);
   N[3].push_back(4); N[3].push_back(5);
   N[4].push_back(3);
   N[5].push_back(2); N[5].push_back(3); N[5].push_back(6);
   N[6].push_back(1); N[6].push_back(5);
   
   // Transition probability matrix
   for ( int i = 0; i < 7; i++ ){
	   for ( int j = 0; j < 7; j++ ){
		   if ( j == i){
			   P[i][j] = 1.0 - (N[i].size() / 7.0);
		   }
		   else{
			   if ( std::count(N[i].begin(), N[i].end(), j) > 0 ){
				   P[i][j] = 1.0 / 7.0;
			   }
			   else{
				   P[i][j] = 0.0;
   ...
   // MISSA object.
   MISSA markov_ram_op(x.size(), nchains, nthreads, mode);
   ...
   for ( int test = 1; test < 9; test++ ){
	   ...
	   for ( int k = 0; k < niters; k++ ){
	      ...
	      // Optimization step
	      markov_ram_op.apply( lambda, next_states, eps, x );
	  ...
	  /*
	  char buffer [500];
	  int str = std::sprintf(buffer, "/home/rmassambone/Documentos/Elias-Eduardo/MISSA-testes/results/diminishing-stepsize/longer_horizon/time_Markov_Ram_test-%d.txt", test );
	  FILE * arq;
	  ...
	  */
	  ...
	  char buffer [500];
	  int str = std::sprintf(buffer, "/home/rmassambone/Documentos/Elias-Eduardo/MISSA-testes/results/constant-stepsize/best-for-each/objective_Markov_Ram_test-%d.txt", test );
	  FILE * arq;
	  arq = std::fopen (buffer, "w" );
	  ...
	  /*
	  char buffer3 [500];
	  int str3 = std::sprintf(buffer3, "/home/rmassambone/Documentos/Elias-Eduardo/MISSA-testes/results/diminishing-stepsize/longer_horizon/time_best_Markov_Ram_test-%d.txt", test );
	  FILE * arq3;
	  ...
	  char buffer4 [500];
	  int str4 = std::sprintf(buffer4, "/home/rmassambone/Documentos/Elias-Eduardo/MISSA-testes/results/diminishing-stepsize/longer_horizon/objective_best_Markov_Ram_test-%d.txt", test );
	  FILE * arq4;
