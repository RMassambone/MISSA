...
int main( int argc, char **argv )
{
   ...
   for ( int test = 1; test < 9; test++ ){
	   ...
	   for (int k = 0; k < niters; k++){
	      ...
	      // Optimization step
	      cyclic_op.apply( lambda, next_states, eps, x );
	  ...
	  */
	  ...
	  char buffer [500];
	  int str = std::sprintf(buffer, "/home/rmassambone/Documentos/Elias-Eduardo/MISSA-testes/results/constant-stepsize/best-for-each/objective_cyclic_test-%d.txt", test );
	  FILE * arq;
	  arq = std::fopen (buffer, "w" );
